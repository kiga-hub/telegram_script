...
def main():
    ...
    check_interval = config.get("check_interval", 20)  # Set default to 5 seconds if not specified
    loop_count = config.get("loop_count", 1000)  # Set default to 10 loops if not specified
    sleep_duration = config.get("sleep_duration", 4)  # Set default to 1 hour if not specified
    account_switch_delay = config.get("account_switch_delay", 20)  # Delay between accounts
    ...
    # 循环N次
    print(f'{Fore.YELLOW}[INFO] Running the bot for {loop_count} loops...')
    for i in range(loop_count):
        print(f'{Fore.YELLOW}[INFO] Loop {i+1}/{loop_count}')
        ...
        print(f'{Fore.YELLOW}[INFO] Waiting for {sleep_duration} seconds before the next loop...')
        time.sleep(sleep_duration)
